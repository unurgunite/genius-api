...
describe Genius::Search, ' .search with valid token and query' do
  let(:token) { 'a' * 64 }
...
describe Genius::Search, ' .search with search_by' do
  let(:token) { 'a' * 64 }
  let(:mock_response) { instance_double(HTTParty::Response, body: response_body) }
  let(:response_body) do
    {
...
describe Genius::Search, ' .search without token' do
  let(:mock_response) { instance_double(HTTParty::Response, body: response_body) }
  let(:response_body) do
    {
      'meta' => { 'status' => 200 },
...
describe Genius::Search, ' .search with nil query' do
  let(:token) { 'a' * 64 }
  let(:mock_response) { instance_double(HTTParty::Response, body: response_body) }
  let(:response_body) do
    {
