...
describe Genius::Errors, ' .validate_token with short token' do
...
describe Genius::Errors, ' .validate_token with valid token and 200 response' do
...
describe Genius::Errors, ' .validate_token with valid token and non-200 response' do
  let(:valid_token) { 'a' * 64 }
  let(:mock_response) { instance_double(HTTParty::Response, body: response_body) }
...
describe Genius::Errors, ' .error_handle? with short token' do
  let(:short_token) { 'short_token' }
...
describe Genius::Errors, ' .error_handle? with valid token' do
  let(:valid_token) { 'a' * 64 }
  let(:mock_response) { instance_double(HTTParty::Response, body: response_body) }
  let(:response_body) { { 'meta' => { 'status' => 200 } }.to_json }
