...
describe Genius::Referents, ' .referents with valid token' do
  let(:token) { 'a' * 64 }
...
describe Genius::Referents, ' .referents with options' do
  let(:token) { 'a' * 64 }
  let(:mock_response) { instance_double(HTTParty::Response, body: response_body) }
  let(:response_body) { { 'meta' => { 'status' => 200 }, 'response' => { 'referents' => [] } }.to_json }
...
describe Genius::Referents, ' .referents with both web_page and song_id' do
  let(:token) { 'a' * 64 }
...
describe Genius::Referents, ' .referents without token' do
  before do
