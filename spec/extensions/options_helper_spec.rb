  ...
  describe '#options_helper' do
    ...
    context 'when options contain allowed keys' do
      it 'builds a query string from allowed options' do
        result = subject.options_helper({ sort: 'title', per_page: 10, page: 1 }, allowed_keys)
    ...
    context 'when options contain disallowed keys' do
      it 'ignores disallowed keys' do
        result = subject.options_helper({ sort: 'title', foo: 'bar' }, allowed_keys)
    ...
    context 'when options is empty' do
      it 'returns an empty string' do
        result = subject.options_helper({}, allowed_keys)
        expect(result).to eq('')
      end
    end
  end
